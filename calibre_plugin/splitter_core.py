...
EPUBSPLIT_PATH = ROOT / "epubsplit.py"
...
def detect_split_ranges(
    ...
    """Detect single-book ranges in an EPUB collection.

    This is the GUI-friendly entry point used by the Calibre plugin. It does
    the same detection work as the CLI but returns a structured report instead
    of printing or writing files.
    """
    epubsplit = load_epubsplit()
    ...
    if use_llm:
        try:
            result = ask_llm(epub_path, lines, expected_count, toc_nodes, vllm_base_url, model, llm_timeout)
            source = f"llm:{result.get('_model', model or 'auto')}"
        except Exception as exc:
            result = heuristic_books(splitter, lines, expected_count)
            result.setdefault("notes", []).append(f"vLLM unavailable or invalid response: {exc}")
    else:
        result = heuristic_books(splitter, lines, expected_count)
