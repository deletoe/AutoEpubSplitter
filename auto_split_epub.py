...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if not args.no_llm and source.startswith("llm:") and not books:
        print("[warn] vLLM returned no valid split points; retrying with heuristic TOC draft", file=sys.stderr)
        ...
        if draft_books:
            try:
                if args.stream_llm:
                    print("\n[retrying with TOC draft]", file=sys.stderr)
                ...
                if retry_books:
                    result = retry_result
                    source = f"llm-retry:{retry_result.get('_model', args.model or 'auto')}"
                    books = retry_books
            except Exception as exc:
                print(f"[warn] vLLM empty-response retry failed: {exc}", file=sys.stderr)
                draft.setdefault("notes", []).append(f"LLM empty-response retry failed: {exc}")
        if not books:
            print("[warn] vLLM returned no valid split points; using heuristic fallback", file=sys.stderr)
